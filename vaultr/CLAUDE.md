...
- `POST /transactions` — body: `{ accountId, type, amount, title|merchantName, description?, category?, recipient? }`. Validates a positive amount and account ownership, atomically updates the account balance (credit adds, debit subtracts), rejects if the result would go negative, and returns `{ transaction, account }`. If `category` is omitted, it's auto-tagged from the merchant/title via keyword matching (`utils/categorize.ts`) into one of `Food | Transport | Entertainment | Shopping | Bills | Income | Transfers | Other`; an explicitly-supplied category is always preserved as-is
